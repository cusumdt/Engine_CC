...
void Input::setWindow(GLFWwindow* _window)
{
	...
	lastMousePos.x = (float)m_viewport[2] / 2;
	lastMousePos.y = (float)m_viewport[3] / 2;
}
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	{
		Input::lastMousePos.x = (float)xpos;
		Input::lastMousePos.y = (float)ypos;
		Input::firstMouseInput = false;
	}

	float xoffset = (float)xpos - Input::lastMousePos.x;
	float yoffset = Input::lastMousePos.y - (float)ypos; // reversed since y-coordinates range from bottom to top
	Input::lastMousePos.x = (float)xpos;
	Input::lastMousePos.y = (float)ypos;
