...
{
	scale = _scale;
	currentRenderer = BaseGame::newRenderer;
	ShaderProgramSource source = currentRenderer.ShaderParser("res/shaders/Shape.shader");
	shader.ID = currentRenderer.CreateShader(source.vertexSource, source.fragmentSource);
	shader.use();
...
void Sprite::draw()
{
	BaseGame::newRenderer.Draw(shader.ID,texture1.GetTexture(),texture2.GetTexture(),VertexArrayID,model,uniModel);
}
